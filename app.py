...
def load_rules():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT * FROM rules ORDER BY province, city")
    rows = dict_fetchall(c)
    conn.close()
    return rows

def load_rules_by_city(city):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT * FROM rules WHERE city=? OR province=?", (city, city))
    rows = dict_fetchall(c)
    conn.close()
    return rows
...
# ========== 全国省份默认规则 ==========
# 基于2024年全国各省社保政策，单位社保比例16%，个人8%为基准，
# 各省有细微差异，已根据实际情况调整
...
def get_rule_for_city(city):
    ...
    # 再找省份匹配（city字段为省份名）
    for r in rules:
        if r.get('province') == city or r['city'] == city:
            return r
    # 最后找省份默认（用province字段）
    for r in rules:
...
def get_province_for_city(city, companies):
    for c in companies:
        if c['city'] == city:
            return c.get('province', city)
    return city
...
def import_rules_from_excel(file):
    try:
        ...
        if new_rules:
            # 合并：保留已有规则，新增或覆盖
            existing = load_rules()
            existing_cities = {r['city'] for r in existing}
            # 删除要覆盖的城市
            new_cities = {r['city'] for r in new_rules}
            keep = [r for r in existing if r['city'] not in new_cities]
            all_rules = keep + new_rules
            save_rules(all_rules)
...
st.info(f"📌 已内置全国31个省份及主要城市的社保公积金规则，共 {len(PROVINCE_DEFAULT_RULES)} 条规则")
...
with st.sidebar:
    ...
    if uploaded_file:
        with st.spinner("正在解析Excel..."):
            ...
            if companies:
                ...
                if rule_count:
                    st.success(f"✅ 成功从「基础配置表」导入 {rule_count} 个城市的规则")
                else:
                    if error:
                        st.info(f"ℹ️ {error}，使用内置默认规则")
                    else:
                        st.info("ℹ️ 使用内置全国规则")
    ...
    with st.sidebar.expander("⚖️ 规则管理（全国覆盖）"):
        rules = load_rules()
        if rules:
            # 显示规则数量
            cities = [r['city'] for r in rules if not r.get('is_default', 0)]
            provinces = [r['city'] for r in rules if r.get('is_default', 0)]
            st.info(f"📌 共 {len(rules)} 条规则（{len(cities)} 个城市 + {len(provinces)} 个省份默认）")
            df_rules = pd.DataFrame(rules)
            st.dataframe(df_rules[['city', 'unit_social', 'personal_social', 'unit_fund', 'personal_fund', 'social_min', 'social_max']])
        else:
            st.info("暂无规则")
...
if selected_companies and report_type:
    ...
    for comp in selected_companies:
        ...
        if rule is None:
            ...
            rule_details.append(f"{comp['city']}: 单位{rule['unit_social']*100:.0f}% 个人{rule['personal_social']*100:.0f}% 公积金{rule['unit_fund']*100:.0f}%")
    
    if missing_rules:
        st.warning(f"⚠️ 以下城市缺少规则，将使用默认值：{', '.join(set(missing_rules))}")
    else:
        st.success("✅ 所有城市已配置规则")
        with st.expander("查看各城市规则"):
